import require$$0$3, { resolve } from 'path';
import require$$0$5, { existsSync, lstatSync, promises } from 'fs';
...
const getNpmConfig = async () => {
  const npmcliConfig = new NpmcliConfig({
    definitions: {},
    npmPath: require$$0$3.dirname(process.cwd()),
  ...
  npmcliConfig.loadDefaults = () => {
    ...
    setCliOption("userconfig", require$$0$3.join(npmcliConfig.home, ".npmrc"));
    setCliOption("globalconfig", require$$0$3.join(npmcliConfig.globalPrefix, "etc", "npmrc"));
  };
...
const cacheDir = resolve(require$$0$1.tmpdir(), "taze");
...
async function resolveDependency(raw, options, filter = () => true) {
  ...
  if (error == null) {
    try {
      target = getVersionOfRange(dep, mergeMode);
    } catch (e) {
      err = e.message || e;
    }
  } else {
...
function dumpDependencies(deps, type) {
  const data = {};
  deps.filter((i) => i.source === type).sort((a, b) => a.name.localeCompare(b.name)).forEach((i) => {
...
async function writeJSON(filepath, data) {
  const actualContent = await promises.readFile(filepath, "utf-8");
...
async function loadPackage(relative, options, shouldUpdate) {
  const filepath = require$$0$3.resolve(options.cwd, relative);
...
async function loadPackages(options) {
  ...
  if (options.recursive) {
    packagesNames = await out("**/package.json", {
      ignore: [
        "**/node_modules/**",
        "**/dist/**",
        "**/public/**"
...
async function CheckPackages(options, callbacks = {}) {
  ...
  const privatePackageNames = packages.filter((i) => i.raw.private).map((i) => i.raw.name).filter((i) => i);
...
export { CheckPackages as C, DependenciesTypeShortMap as D, _debug as _, resolveDependencies as a, resolveDependency as b, loadPackages as c, dumpDependencies as d, DiffMap as e, getPrefixedVersion as f, getVersionOfRange as g, getPackageData as h, loadPackage as l, parseDependencies as p, resolvePackage as r, toArray as t, updateTargetVersion as u, writePackage as w };
