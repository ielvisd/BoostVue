...
function guessEditor() {
  ...
  try {
    if (process.platform === "darwin") {
      const output = child_process.execSync("ps x", { stdio: ["pipe", "pipe", "ignore"] }).toString();
      ...
      const output = child_process.execSync('powershell -Command "Get-Process | Select-Object Path"', {
      ...
      const output = child_process.execSync("ps x --no-heading -o comm --sort=comm", { stdio: ["pipe", "pipe", "ignore"] }).toString();
...
function printInstructions(fileName, errorMessage) {
  console.log();
  console.log(chalk.red(`Could not open ${path2.basename(fileName)} in the editor.`));
  if (errorMessage) {
  ...
  console.log();
  console.log(`To set up the editor integration, add something like ${chalk.cyan("VUE_EDITOR=atom")} to the ${chalk.green(".env.local")} file in your project folder and restart the development server. Learn more: ${chalk.green("https://goo.gl/MMTaZt")}`);
  console.log();
}
...
function getInspectorPath() {
  const pluginPath = normalizePath(path3.dirname(fileURLToPath(import.meta.url)));
    ...
    async load(id) {
      if (id === "virtual:vue-inspector-options") {
        ...
        if (fs2.existsSync(file))
          return await fs2.promises.readFile(file, "utf-8");
        else
