...
const pc__default = /*#__PURE__*/_interopDefaultLegacy(pc);
...
function createBridgeCodeGenerator(source, query) {
  return () => {
    ...
    if (shared.isString(query.locale)) {
...
function convert(source, lang) {
  switch (lang) {
    case "yaml":
    case "yml":
      const data = yaml__default.load(source);
      return JSON.stringify(data, void 0, "	");
    case "json5":
      return JSON.stringify(JSON5__default.parse(source));
    default:
      return source;
  }
}
...
function warn(...args) {
  console.warn(pc__default.yellow(pc__default.bold(`[unplugin-vue-i18n] `)), ...args);
}
function error(...args) {
  console.error(pc__default.red(pc__default.bold(`[unplugin-vue-i18n] `)), ...args);
}
...
const debug = createDebug__default("unplugin-vue-i18n");
const installedPkg = bundleUtils.checkInstallPackage("@intlify/unplugin-vue-i18n", debug);
const installedVueI18nBridge = bundleUtils.checkVueI18nBridgeInstallPackage(debug);
const vueI18nVersion = bundleUtils.getVueI18nVersion(debug);
const unplugin = unplugin$1.createUnplugin((options = {}, meta) => {
  ...
  const getVueI18nAliasName = () => vueI18nVersion === "9" || vueI18nVersion === "8" ? "vue-i18n" : vueI18nVersion === "unknown" && installedPkg === "petite-vue-i18n" && shared.isBoolean(useVueI18nImportName) && useVueI18nImportName ? "vue-i18n" : installedPkg;
  ...
  const esm = shared.isBoolean(options.esm) ? options.esm : true;
  debug("esm", esm);
    ...
    vite: {
      config(config, { command }) {
        ...
        if (command === "build" && runtimeOnly) {
          ...
          } else if (shared.isObject(config.resolve.alias)) {
...
async function getCode(source, filename, sourceMap, query, framework = "vite") {
  ...
  if (framework === "webpack") {
    if (issuerPath) {
      ...
      const result = compilerSfc.parse(await getRaw(filename), {
        sourceMap,
        filename
      });
